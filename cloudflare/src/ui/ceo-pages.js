...
import { dashboardCssHref } from './ceo-static.js';
...
//
//   1. new Intl.DateTimeFormat() is ~50-100µs PER CONSTRUCTION. The old
//      code constructed one per format call inside the active-session
//      loop, so 5 active sessions × 2 calls = 10 constructions per
//      render = ~18,000 per hour. Cache by (tz, pattern-name) and
//      reuse the same instance. Drops to ~5µs per call.
//
...
function buildLiveSessionsHtml() {
  ...
  return activeIds
    .map(function (id) {
      ...
      // v1.2.28: cached Intl.DateTimeFormat (was ~50-100µs per call,
      // now ~5µs). Same output as the old toLocaleString form.
