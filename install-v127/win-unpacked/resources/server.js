...
/**
 * Production / factory timezone (IANA). All "Check Report" date boundaries,
 * weekday labels, and per-day buckets are interpreted in this zone — never
 * the browser's local zone. Defaults to Dubai because the factory operates
 * there; override with FACTORY_TZ in .env if operations move.
 */
const FACTORY_TZ = String(process.env.FACTORY_TZ || 'Asia/Dubai').trim() || 'Asia/Dubai';

/**
 * Persistent store for manually recorded cancellations. Cancellations are a
 * first-class operational state — they live next to the production log so
 * the "Check Report" view can show real, queryable cancellation records
 * rather than inferring "cancelled = total - delivered - pending".
 *
 * Each row: { id, factory, invoiceNo, abayaCode, reason, cancelledBy,
 *            cancelledAt (ms), source }
 */
const CANCELLATIONS_FILE = path.join(DATA_DIR, 'manual-cancellations.json');
let CANCELLATIONS = [];
function _loadCancellationsFromDisk() {
  try {
    if (!fs.existsSync(CANCELLATIONS_FILE)) return [];
    const raw = fs.readFileSync(CANCELLATIONS_FILE, 'utf8');
    const arr = JSON.parse(raw);
    return Array.isArray(arr) ? arr : [];
  } catch (e) {
    console.warn('[cancellations] load failed — starting empty:', e.message);
    return [];
  }
}
function _saveCancellationsToDisk() {
  try {
    fs.mkdirSync(path.dirname(CANCELLATIONS_FILE), { recursive: true });
    fs.writeFileSync(CANCELLATIONS_FILE, JSON.stringify(CANCELLATIONS, null, 2), 'utf8');
  } catch (e) {
    console.warn('[cancellations] persist failed:', e.message);
  }
}
CANCELLATIONS = _loadCancellationsFromDisk();

// ─── Cancellations store helpers ──────────────────────────────────────────────
// Pure cancellation CRUD. The aggregation logic lives in shared/check-report.cjs
// so the test harness can exercise it without booting the HTTP server.

/**
 * Default factory name. Used when a cancellation arrives without a factory
 * and when an abaya row has no factory field. Override with the
 * DEFAULT_FACTORY env var.
 */
const DEFAULT_FACTORY = String(process.env.DEFAULT_FACTORY || 'Main Factory').trim() || 'Main Factory';

/**
 * Look up factory for an abaya_id from the catalog. Used by the
 * /api/check-report/config endpoint to enumerate distinct factory
 * names and to mark the "default" factory when an abaya has none.
 */
function factoryForAbaya(abayaId) {
  if (abayaId == null) return DEFAULT_FACTORY;
  const a = abayaCatalog.find((x) => x && x.id === abayaId);
  if (!a) return DEFAULT_FACTORY;
  return String(a.factory || '').trim() || DEFAULT_FACTORY;
}

/**
 * Record a new manual cancellation. Pure: does not derive cancellation from
 * math. Returns the persisted record.
 */
function recordCancellation(input) {
  const cancelledAt = Number(input && input.cancelledAt) > 0
    ? Number(input.cancelledAt)
    : Date.now();
  const rec = {
    id: String((input && input.id) || ('CNL-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e6).toString(36))),
    factory: String((input && input.factory) || DEFAULT_FACTORY).trim() || DEFAULT_FACTORY,
    invoiceNo: input && input.invoiceNo != null ? String(input.invoiceNo).trim() : '',
    abayaCode: input && input.abayaCode != null ? String(input.abayaCode).trim() : '',
    reason: input && input.reason != null ? String(input.reason).trim() : '',
    cancelledBy: input && input.cancelledBy != null ? String(input.cancelledBy).trim() : '',
    cancelledAt,
    source: String((input && input.source) || 'manual'),
  };
  CANCELLATIONS.push(rec);
  _saveCancellationsToDisk();
  return rec;
}

/**
 * List cancellations within an inclusive [startMs, endMs] window.
 * Sorted newest-first by cancelledAt.
 */
function listCancellations({ startMs, endMs, factory } = {}) {
  const s = Number(startMs);
  const e = Number(endMs);
  if (!Number.isFinite(s) || !Number.isFinite(e)) return [];
  const fac = factory ? String(factory).trim() : '';
  return CANCELLATIONS
    .filter((c) => c && c.cancelledAt >= s && c.cancelledAt <= e)
    .filter((c) => !fac || String(c.factory) === fac)
...
/**
 * GET /api/check-report?from=YYYY-MM-DD&to=YYYY-MM-DD&factory=Factory%20A
 *
 * Production Throughput Check Report. Aggregates COMPLETED_LOGS +
 * ACTIVE_SESSIONS + manual_cancellations into a Factory → Abaya Invoice →
 * Abaya Code → Status → Date/Time report, using FACTORY_TZ for all
 * date boundaries (never the browser's local zone).
 *
 * Query params:
 *   from, to   — YYYY-MM-DD (both inclusive). If only `from` is given,
 *                the report covers that single day.
 *   factory    — optional factory name; if set, the report scopes to that
 *                factory only.
 */
app.get('/api/check-report', function (req, res) {
  res.setHeader('Cache-Control', 'no-store');
  const fromYmd = checkReport.normalizeYmd(req.query.from);
  const toYmd   = checkReport.normalizeYmd(req.query.to) || fromYmd;
  if (!fromYmd || !toYmd) {
    return res.status(400).json({
      ok: false,
      error: 'invalid-date',
      hint: 'Pass from=YYYY-MM-DD (and optional to=YYYY-MM-DD).',
    });
  }
  const factory = req.query.factory ? String(req.query.factory) : '';
  const report = checkReport.aggregateCheckReport({
    fromYmd, toYmd, factory,
    timezone: FACTORY_TZ,
  ...
  if (!report.ok) {
    return res.status(400).json({ ok: false, error: report.error });
  }
  return res.json(report);
});

/**
 * GET /api/cancellations?from=YYYY-MM-DD&to=YYYY-MM-DD&factory=...
 * List manually recorded cancellations in a date range (default: today
 * in FACTORY_TZ). Newest first.
 */
app.get('/api/cancellations', function (req, res) {
  res.setHeader('Cache-Control', 'no-store');
  const tz = FACTORY_TZ;
  let fromYmd = checkReport.normalizeYmd(req.query.from);
  let toYmd   = checkReport.normalizeYmd(req.query.to) || fromYmd;
  if (!fromYmd) {
    fromYmd = checkReport.ymdInTimezone(Date.now(), tz);
    toYmd   = fromYmd;
  }
  const bounds = checkReport.resolveDateRangeBounds(fromYmd, toYmd, tz);
  if (!bounds) return res.status(400).json({ ok: false, error: 'invalid-date-range' });
  const factory = req.query.factory ? String(req.query.factory) : '';
  return res.json({
    ok: true,
    timezone: tz,
    dateRange: {
      from: bounds.fromYmd,
      to: bounds.toYmd,
      fromLong: checkReport.longDateInTimezone(bounds.fromYmd, tz),
      toLong: checkReport.longDateInTimezone(bounds.toYmd, tz),
    },
    cancellations: listCancellations({ startMs: bounds.startMs, endMs: bounds.endMs, factory }),
  });
});

/**
 * POST /api/cancellations
 * Record a new manual cancellation. Body: { factory, invoiceNo?, abayaCode?,
 *   reason?, cancelledBy?, cancelledAt? }.
 *
 * Cancellation is a first-class operational state. We do not infer it
 * from math; this endpoint is the only way to mark an abaya as
 * cancelled in the report.
 */
app.post('/api/cancellations', function (req, res) {
  res.setHeader('Cache-Control', 'no-store');
  const body = req.body || {};
  if (!body || typeof body !== 'object') {
    return res.status(400).json({ ok: false, error: 'invalid-body' });
  }
  // Soft validation: at least one of invoiceNo / abayaCode must be set so
  // the cancellation is traceable back to an operational record. The spec
  // requires every cancellation to remain connected to the invoice / code.
  ...
  const rec = recordCancellation({
    factory: body.factory,
    invoiceNo: body.invoiceNo,
    abayaCode: body.abayaCode,
    reason: body.reason,
    cancelledBy: body.cancelledBy,
    cancelledAt: body.cancelledAt,
    source: body.source || 'manual',
  });
  return res.status(201).json({ ok: true, cancellation: rec });
});

/**
 * GET /api/check-report/config
 * Surfaces the production timezone + default factory so the UI can show
 * "Generated in Asia/Dubai" and pre-fill the factory filter.
 */
app.get('/api/check-report/config', function (req, res) {
  res.setHeader('Cache-Control', 'no-store');
  // Distinct factory names derived from the in-memory catalog + recorded
  // cancellations. Useful for populating a factory filter dropdown.
  const factories = new Set();
  for (const a of abayaCatalog) {
    const f = String(a && a.factory || '').trim();
    if (f) factories.add(f);
  }
  for (const c of CANCELLATIONS) {
    const f = String(c && c.factory || '').trim();
    if (f) factories.add(f);
  }
  ...
  return res.json({
    ...
    todayYmd: checkReport.ymdInTimezone(Date.now(), FACTORY_TZ),
  });
});
