...
// ============================================================
// AGENTS.md §1 contract: emp_id MUST be in the form e_bc_<digits>. The
// previous in-source DEFAULT_EMPLOYEES hardcoded synthetic ids e1..e26
// that violated this contract and were rejected by the cloud's migration
// 0018 BEFORE INSERT trigger with 422 'emp_id must be in the form
// e_bc_<barcode>'. The hardcoded roster has been removed; EMPLOYEES is
// now populated at boot from either:
//   1. local employees.xlsx (EMPLOYEES_XLSX_PATH env), or
//   2. cloud /api/employees pull (refreshEmployeesFromCloud, every
//      ROSTER_PULL_MS), or
//   3. employees-manual.json (manual kiosk edits, see loadEmployeesFromXlsxFile
//      fallback chain).
// Until one of those populates it, EMPLOYEES is empty and the kiosk shows
// no role buttons — which is the correct pre-roster state. The cloud is
// the source of truth for emp_id format per AGENTS.md §1.
let EMPLOYEES = [];
